...
const campaign = {
  ...
  status: "RUNNING",
  totalRecipients: 2,
  variableMappings: { name: "firstName" },
  template: {
    status: "APPROVED",
...
test("worker processes recipients independently and persists message IDs", async (t) => {
  ...
  const recipients = [
    {
      id: "recipient-a",
      customer: { id: "customer-a", mobile: "1111111111", firstName: "Asha" },
    },
    {
      id: "recipient-b",
      customer: { id: "customer-b", mobile: "2222222222", firstName: "Ravi" },
    },
  ];
  const updates = [];
  ...
  stub(
    ...
    async (_connection, request) => {
      if (request.to === "1111111111")
        throw new Error("Meta rejected recipient");
      return { messages: [{ id: "wamid-ravi" }] };
    },
    cleanup,
  );
  ...
  assert.ok(
    updates.some(
      (item) => item.id === "recipient-a" && item.data.status === "FAILED",
    ),
  );
  assert.ok(
    updates.some(
      (item) =>
        item.id === "recipient-b" &&
        item.data.whatsappMessageId === "wamid-ravi",
    ),
  );
...
test("worker rejects a disconnected tenant connection", async (t) => {
  ...
  t.after(() => {
    whatsappRepository.findByCompanyId = original;
    campaignRepository.findById = originalCampaign;
  });
  campaignRepository.findById = async () => campaign;
  whatsappRepository.findByCompanyId = async () => ({ status: "DISCONNECTED" });
  await assert.rejects(
    worker.process("company-a", "campaign-a"),
...
test("worker rejects media that is not owned by the tenant", async (t) => {
  ...
  stub(
    campaignRepository,
    "findById",
    async () => ({ ...campaign, mediaId: "foreign-media" }),
    cleanup,
  );
  stub(
    mediaService,
...
test("worker rejects unresolved required variables", async () => {
  await assert.rejects(
    worker.processRecipient(
      "company-a",
      connection,
      campaign,
      { id: "recipient-a", customer: { mobile: "1111111111", firstName: "" } },
      null,
      null,
    ),
