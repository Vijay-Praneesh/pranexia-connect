...
class LocalStorageProvider {
  ...
  resolve(storageKey) {
    ...
    if (
      ...
      !target.startsWith(`${this.rootPath}${path.sep}`)
    )
  ...
  async delete(storageKey) {
    try {
      await fsp.unlink(this.resolve(storageKey));
    } catch (error) {
  ...
  async exists(storageKey) {
    try {
      await fsp.access(this.resolve(storageKey));
      return true;
    } catch {
      return false;
    }
...
class StorageService {
  constructor() {
    if (mediaConfig.storageProvider !== "local") {
      throw new Error(
        `MEDIA_STORAGE_PROVIDER=${mediaConfig.storageProvider} is not configured. Add an object-storage provider implementation before selecting it.`,
      );
    }
  ...
  readBuffer(key) {
    return this.provider.readBuffer(key);
  }
  delete(key) {
