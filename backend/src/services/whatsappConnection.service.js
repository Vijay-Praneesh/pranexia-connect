...
class WhatsAppConnectionService {
  get apiVersion() {
    return process.env.META_API_VERSION || process.env.WHATSAPP_API_VERSION;
  }
  get graphBase() {
  ...
  async getStatus(companyId) {
    ...
    return {
      ...
      connection: this.safeConnection(connection),
    };
  }
  ...
  async connect(companyId, { code, wabaId, phoneNumberId }) {
    ...
    if (
      ...
      throw new AppError("Meta WhatsApp onboarding is not configured", 503);

    let token;
    try {
      const exchange = await axios.get(`${this.graphBase}/oauth/access_token`, {
        params: {
      ...
      const [waba, phone] = await Promise.all([
        axios.get(`${this.graphBase}/${wabaId}`, {
          params: { fields: "id,name", access_token: token },
        }),
        axios.get(`${this.graphBase}/${phoneNumberId}`, {
          params: {
      ...
      if (
        String(waba.data.id) !== String(wabaId) ||
        String(phone.data.id) !== String(phoneNumberId)
      )
        throw new AppError(
          "Meta returned mismatched WhatsApp identifiers",
          400,
        );
      await axios.post(`${this.graphBase}/${wabaId}/subscribed_apps`, null, {
        params: { access_token: token },
      });
      const connection = await whatsappRepository.upsert(companyId, {
      ...
      throw new AppError(
        error.response?.data?.error?.message ||
          "Meta WhatsApp connection could not be verified",
        502,
      );
    }
  }
  ...
  async disconnect(companyId) {
    const connection = await whatsappRepository.disconnect(companyId);
    return connection ? this.safeConnection(connection) : null;
  }
