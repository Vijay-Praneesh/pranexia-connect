...
export class CampaignsComponent implements OnDestroy {
  ...
  private readonly errors = inject(HttpErrorService); private readonly route = inject(ActivatedRoute); private readonly router = inject(Router); private readonly fb = inject(FormBuilder);
  private readonly destroy$ = new Subject<void>(); private pollHandle?: ReturnType<typeof setInterval>;

  campaigns: Campaign[] = []; approvedTemplates: Template[] = []; customers: Customer[] = []; selectedCustomers = new Set<string>();
  ...
  readonly campaignForm = this.fb.nonNullable.group({ name: ['', [Validators.required, Validators.maxLength(150)]], description: [''], templateId: ['', Validators.required], sendType: ['NOW' as CampaignSendType, Validators.required], scheduledAt: [''] });
  ...
  loadOptions(): void {
    forkJoin({ templates: this.templatesApi.getTemplates({ page: 1, limit: 100, status: 'APPROVED', sortBy: 'name', order: 'ASC' }), customers: this.customersApi.getCustomers({ page: 1, limit: 100, status: 'ACTIVE', sortBy: 'firstName', order: 'ASC' }) }).subscribe({ next: ({ templates, customers }) => { this.approvedTemplates = templates.templates; this.customers = customers.customers; }, error: (error) => { this.errorMessage = this.errors.map(error).message; } });
  }
  ...
  openCreate(): void { this.editing = null; this.selectedCustomers.clear(); this.formError = ''; this.campaignForm.reset({ name: '', description: '', templateId: '', sendType: 'NOW', scheduledAt: '' }); this.editorOpen = true; }
  openEdit(campaign: Campaign): void { this.editing = campaign; this.selectedCustomers.clear(); this.formError = ''; this.campaignForm.reset({ name: campaign.name, description: campaign.description ?? '', templateId: campaign.templateId, sendType: campaign.sendType, scheduledAt: campaign.scheduledAt ? this.toLocalInput(campaign.scheduledAt) : '' }); this.editorOpen = true; }
  ...
  save(): void {
    ...
    const payload: CampaignWriteRequest = { name: raw.name.trim(), description: raw.description.trim() || null, templateId: raw.templateId, sendType: raw.sendType, scheduledAt };
