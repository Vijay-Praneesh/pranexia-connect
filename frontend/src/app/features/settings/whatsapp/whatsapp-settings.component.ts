...
export class WhatsAppSettingsComponent implements OnInit, OnDestroy {
  ...
  connect(): void {
    ...
    this.loadMetaSdk(() => {
      if (!window.FB) {
        this.failConnection('Meta Embedded Signup could not be loaded.');
        return;
      }
      window.FB.login(
        (response) => {
          if (!response.authResponse?.code)
            this.failConnection(
              'WhatsApp onboarding was cancelled or did not return an authorization code.',
            );
    ...
    this.api
      ...
      .subscribe({
        next: () => {
          ...
          this.successMessage = 'WhatsApp connection disconnected.';
        },
        error: (error: unknown) => {
          this.errorMessage = this.errors.map(error).message;
        },
    ...
    this.api
      ...
      .subscribe({
        next: (connection) => {
          ...
          this.successMessage = 'WhatsApp Business connected successfully.';
        },
        error: (error: unknown) => {
          this.failConnection(this.errors.map(error).message);
        },
      });
    this.signupCode = '';
  }
  ...
  private loadMetaSdk(ready: () => void): void {
    ...
    script.onload = () => {
      window.FB?.init({
        ...
        cookie: true,
        xfbml: true,
        version: environment.metaApiVersion,
      });
      ready();
    };
    script.onerror = () =>
